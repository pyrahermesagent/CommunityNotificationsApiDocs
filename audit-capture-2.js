...
;(async () => {
  await fs.promises.mkdir('/tmp/audit2', { recursive: true })
  ...
  if (!ok) {
    console.log('Server failed to start, killing and retrying...')
    server.kill()
    await new Promise(r => setTimeout(r, 1000))
    const server2 = spawn('node', ['test-server.js'], { stdio: ['pipe', 'pipe', 'pipe'] })
    await new Promise(r => setTimeout(r, 2000))
  }
  ...
  for (const p of pages) {
    for (const vp of viewports) {
      ...
      try {
        ...
        const outFile = `/tmp/audit2/${p.name}_${vp.name}.png`
